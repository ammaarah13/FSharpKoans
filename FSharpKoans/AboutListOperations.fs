   ...
   *)
    ...
    let ``07 Specified-function mapping, the hard way`` () =
        let map (f : 'a -> 'b) (xs : 'a list) : 'b list =
          let rec maplist f acc = // write a map which applies f to each element
               match acc with
               | [] -> []  
               | a::b -> (f a) :: maplist f b
          match xs with 
    ...
    let ``09 Specified-function filtering, the hard way`` () =
        let filter (f : 'a -> bool) (xs : 'a list) : 'a list =
            let rec filterlist f list = // write a function which filters based on the specified criteria
                match list with 
                |a::b -> match (f a) with
    ...
    let ``11 Fixed-function filtering, the hard way`` () =
        let filter (xs : int list) : int list =
            let rec filterlist f list = // write a function to filter for odd elements only.
                match list with 
                |a::b -> match (f a) with
                     |true -> a:: filterlist f b   
                     |false -> filterlist f b
...
*)
    ...
    let ``14 A fold which sums a list`` () =
        let fold initialState xs =
            let rec foldfunc list acc =  // write a function to do what's described above
                match list with 
    ...
    let ``15 A fold which multiplies a list`` () =
        let fold initialState xs =
            let rec foldfunc list acc = // write a function to multiply the elements of a list
               match list with 
                |a::b -> foldfunc b (acc*a)
                |_ -> acc         
            match initialState with 
    ...
    let ``18 exists: finding whether any matching item exists`` () =
        let exists (f : 'a -> bool) (xs : 'a list) : bool =
          let rec existlist list  = // Does this: https://msdn.microsoft.com/en-us/library/ee370309.aspx
              match list with
              |[] -> false
              |a::b -> match f a with
                       |false -> existlist b
