...
=======
>>>>>>> 5145f958534db5569f333c61f985791a22414fb1
   ...
   *)
    ...
    let ``08 An identifier pattern will match anything`` () =
        let x = 13 // replace with an integer
        let y = "thirteen" // replace with a string
        let z = 13.0 // replace with anything else!
        x |> should be ofType<int>
        y |> should be ofType<string>

    [<Test>]
    let ``09 A wildcard pattern will match anything`` () =
        let _ = 13 // replace with an integer
        let _ = "thirteen" // replace with a string
        let _ = 13.0 // replace with anything else!
        ()
    ...
    let ``11 Constant patterns fail if the sides don't match exactly`` () =
        (fun () ->
            let "I'm a string" = "I'm another string"
            ()
